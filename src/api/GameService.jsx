    ...
    _transformData: (plays) => {
        return plays
            ...
            .map(p => ({
                id: p.sortOrder,
                type: p.typeDescKey,
                // On utilise l'opérateur de coalescence nulle (??) pour plus de robustesse (comme en TS)
                playerId: p.typeDescKey === 'goal' ?
                    p.details?.scoringPlayerId ?? "Inconnu" :
                    p.details?.shootingPlayerId ?? "Inconnu",
